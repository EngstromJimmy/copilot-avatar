...
- 2026-05-16T23:33:39.835+02:00 — If `subagent.started` arrives with blank or placeholder identity fields, cache recent Copilot `subagent.selected` metadata as a short-lived hint, bind it to the concrete runtime `agentId` on start, and reuse that per-agent hint for later UI sync. Treat `subagent.selected` as metadata-only enrichment; it must never create or gate visible cards.
- 2026-05-16T23:33:39.835+02:00 — Scribe UI name-loss seam identified: In `syncKnownSubagents()`, metadata is lost when `upsertSubagentState()` is called with only agentId during rehydration after context change or window reopen. The `resolveSubagentState()` function cannot recover displayName without cached event.data, so `buildSubagentPayload()` fallback collapses to raw agentId. Fix target: preserve or pass cached metadata to upsertSubagentState() so displayName is not lost in the fallback chain.
